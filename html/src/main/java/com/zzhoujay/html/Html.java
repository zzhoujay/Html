...
public class Html {
    ...
    private static String getTextDirection(Spanned text, int start, int end) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2 && TextDirectionHeuristics.FIRSTSTRONG_LTR.isRtl(text, start, end - start)) {
            return " dir=\"rtl\"";
        } else {
            return " dir=\"ltr\"";
        }
        ...
        for (int i = start; i < end; i = next) {
            ...
            for (int j = 0; j < style.length; j++) {
                ...
                if (style[j] instanceof AbsoluteSizeSpan) {
                    ...
                    if (!s.getDip()) {
                        Application application = Kit.currentApplication();
                        if (application != null) {
